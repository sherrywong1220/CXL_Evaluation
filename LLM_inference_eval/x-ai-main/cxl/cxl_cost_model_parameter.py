...
## this is for cxl, no nvme offloading. Thanks.
...
input_data = [
    {'bls': 1, 'gbs': 1, 'wc': 0.8869359, 'hc': 0, 'cg': 1, 'cc': 0, 'n': 256, 'throughput':  0.131, 'prompt_len' : 2048},

    {'bls': 1, 'gbs': 1, 'wc': 1, 'hc': 0, 'cg': 1, 'cc': 0, 'n': 256, 'throughput':  0.130, 'prompt_len' : 2048},

    {'bls': 1, 'gbs': 1, 'wc': 1, 'hc': 0, 'cg': 0, 'cc': 1, 'n': 256, 'throughput':  0.135, 'prompt_len' : 2048},
    {'bls': 2, 'gbs': 2, 'wc': 1, 'hc': 0, 'cg': 0, 'cc': 1, 'n': 256, 'throughput':  0.235, 'prompt_len' : 2048},
    {'bls': 4, 'gbs': 4, 'wc': 1, 'hc': 0, 'cg': 0, 'cc': 1, 'n': 256, 'throughput':  0.402, 'prompt_len' : 2048},

    {'bls': 4, 'gbs': 4, 'wc': 1, 'hc': 1, 'cg': 0, 'cc': 1, 'n': 256, 'throughput':  0.396, 'prompt_len' : 2048},
    {'bls': 2, 'gbs': 2, 'wc': 1, 'hc': 1, 'cg': 0, 'cc': 1, 'n': 256, 'throughput':  0.235, 'prompt_len' : 2048},
    {'bls': 4, 'gbs': 4, 'wc': 1, 'hc': 0, 'cg': 0.23, 'cc': 0.77, 'n': 256, 'throughput':  0.400, 'prompt_len' : 2048},
...
# Latency / Time
targets = [
   1954.450,  # Corresponding target for the first data point
...
class ComputationModel(nn.Module):
    def __init__(self, config):
        ...
        self.ctog_bdw = nn.Parameter(torch.tensor(13.43493938446045))
        ...
        self.mm_flops_p = nn.Parameter(torch.tensor(12.46887493133545))
        self.mm_flops_g = nn.Parameter(torch.tensor(4.300000190734863))
        self.bmm_flops_p = nn.Parameter(torch.tensor(0.06149693578481674))
    ...
    def forward(self, bls, gbs, wc, hc, cg, cc, prompt_len, n):
        ...
        ctogp = (1 / (self.ctog_bdw * GB)) * (wi * (wc)
                     + 2 * s * h1 * bls * (hc))
        
        gtocp = (1 / (self.gtoc_bdw_cache * GB)) * (4 * (s + 1) * h1 * bls * (cc)) \
                   + (1 / (self.gtoc_bdw_hidden * GB)) * 2 * s * h1 * bls * (hc)

        # compp = gpu_compp
        compp = (1 / (self.mm_flops_p * T)) * bls * (8 * s * h1 ** 2  + 4 * s * h1 * h2) \
                     + (1 / (self.bmm_flops_p * T)) * 4 * bls * s ** 2 * h1

        ctogg = (1 / (self.ctog_bdw * GB)) * (wi * (wc)
                     + 2 * h1 * bls * (hc))

        # gtocg = hidden_gtocg / gtoc_bdw
        gtocg = (1 / (self.gtoc_bdw_hidden * GB)) * 2 * h1 * bls * (hc)

        compg = (1 / (self.mm_flops_g * T)) * bls * (8 * h1 ** 2  + 4 * h1 * h2) \
                     + (1 / (self.bmm_flops_g * T)) * 4 * bls * (s + n / 2) * h1 * cg \
                     + (1 / (self.cpu_flops * T)) * 4 * bls * (s + n / 2) * h1 * (cc)
        return max([ctogp, gtocp, compp]) * l + max([ctogg, gtocg, compg]) * (n - 1) * l
...
if __name__ == "__main__":
    ...
    # {'bls': 0.427 / 64, 'gbs': 1, 'ctog_bdw': 0.000107 * GB, 'wc': 0, 'hc': 0, 'gtoc_bdw_hidden': 0.000107 * GB, 'cg': 100, 'cc': 0, 'n': 64, 'throughput': 0.427, 'compress_w' : True, 'compress_cache' : True}
            ...
            # Forward pass
            output = model(bls, gbs, wc, hc, cg, cc, prompt_len, n)  # Pass the tensors to the model
            # Calculate loss

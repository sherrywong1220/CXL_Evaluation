...
class CostModelConfig:
    ...
    # hardware constants
    ctog_bdw: float = 13.43493938446045 * GB
    gtoc_bdw_cache: float = 0.9700000286102295 * GB
    gtoc_bdw_hidden: float = 3.7059247493743896 * GB

    mm_flops_p: float = 12.46887493133545 * T
    mm_flops_g: float = 4.300000190734863 * T
    bmm_flops_p: float = 0.06149693578481674 * T
    bmm_flops_g: float = 0.07900000363588333 * T
    cpu_flops: float = 0.012299999594688416 * T
...
#    prob += Tpre >= compp
    ...
    prob += ctogp == (1 / ctog_bdw) * (wi * (wc)
                     + 2 * s * h1 * bls * (hc))

    # gtocp = (cache_gtocp + hidden_gtocp) / gtoc_bdw
    prob += gtocp == (1 / gtoc_bdw_cache) * (4 * (s + 1) * h1 * bls * (cc)) \
                   + (1 / gtoc_bdw_hidden) * 2 * s * h1 * bls * (hc)
    ...
    prob += ctogg == (1 / ctog_bdw) * (wi * (wc)
                     + 2 * h1 * bls * (hc))

    # gtocg = hidden_gtocg / gtoc_bdw
    prob += gtocg == (1 / gtoc_bdw_hidden) * 2 * h1 * bls * (hc)
    ...
    prob += compg == (1 / mm_flops_g) * bls * (8 * h1 ** 2  + 4 * h1 * h2) \
                     ...
                     + (1 / cpu_flops_real) * 4 * bls * (s + n / 2) * h1 * (cc)
    ...
    prob += cpu_w_g == 2 * nh * (s + n) * gbs + 2 * h1 * gbs
...
if __name__ == "__main__":
    ...
    # config.l = llama_config.num_hidden_layers
    # config.h1 = llama_config.input_dim
    # config.h2 = llama_config.intermediate_size
    # config.nh = llama_config.n_head

...
class LlamaConfig:
    ...
    #     dense_num_params = vocab_size * hidden_size + \
    # num_hidden_layers * ( hidden_size * hidden_size * 4 + hidden_size * intermediate_size * ffn_factor + hidden_size * 2) + \
...
def download_llama_weights(model_name, path):
    ...
    while True:
        try:
            folder = snapshot_download(hf_model_name, allow_patterns="*.bin", resume_download=True)
            break
    ...
    for bin_file in tqdm(bin_files, desc="Convert format"):
        ...
        for name, param in tqdm(state.items(), leave=False):
            ...
            with open(param_path, "wb") as f:
                np.save(f, param.cpu().detach().numpy())
                # np.save(f, param.to(torch.float).cpu().detach().numpy())


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--model", type=str, default="llama-65b")
    parser.add_argument("--path", type=str, default="/dev/shm/data/llama_weights")

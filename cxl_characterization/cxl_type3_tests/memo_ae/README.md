...
### Fill in the `../util_scripts/env.sh`
1. Set `CLOSEST_NODE` -- from mlc output in `results/mlc.txt`, find the NUMA node that has the lowest latency to CXL memory
    - `sudo lspci -vvv` should also show the NUMA node that a CXL device attaches to.
2. Set `CLOSEST_CORE` -- `numactl -H`, place the first core number of the `CLOSEST_NODE` in `CLOSEST_CORE`. For example, if CXL is connected to NUMA node 1, please place the first CPU in `node 1 cpus: XX, XX+1` (XX) in `CLOSEST_CPU`. 
3. Set `TSC_FREQ` -- The output from `Test Turbo stat` in `results/turbostat.txt` should look like this: 
```
...
| o | Operation | 0 (Load) | 0 - 4 | <ul><li>0 = Load</li><li>1 = NT-load</li><li>2 = Store</li><li>3 = Nt-store|
| B | Flush before block access latency test | NOT flushed | -B | For `-T 3`, this argument decides whether the 64KB region to be access will be flushed. | 
| C | Number of `nop` blocks before block access latency test | 0 | 0 - X | For `-T 3`, this argument decides how many blocks of `nop` should be issued after the cacheline flushes (if there's any) and before the test begines.  |
